...
for(i in 1:length(orders)){
  for(j in 1:100){
    ...
    if(!dir.exists(paths = paste("../mod.comp/", orders[i],"/",j, sep = ""))){
      dir.create(path = paste("../mod.comp/", orders[i],"/",j, sep = ""), 
                 recursive = T)
    }
    
    write.tree(phy = tree.pruned, 
               file = paste("../mod.comp/", orders[i], "/", j, "/", orders[i],".tre", sep = ""))
    ...
    write.table(counts, 
                file = paste("../mod.comp/", orders[i],"/", j, "/", orders[i],".count.txt", sep = ""),
                quote = F,
                sep = "\n",
                row.names = F,
                col.names = F)
    ...
    make.control(mainType = "Run_Fix_Param",
                 outDir = paste("../mod.comp/", orders[i],"/",j,"/","results", sep = ""),
                 dataFile = paste("../mod.comp/", orders[i],"/", j, "/", orders[i],".count.txt", sep = ""),
                 treeFile = paste("../mod.comp/", orders[i], "/", j, "/", orders[i],".tre", sep = ""),
                 maxChrNum = rng[2],
                 minChrNum = rng[1],
                 branchMul = 1,
                 simulationsNum = nsim,
                 ...
                 control.path = paste("../mod.comp/", orders[i],"/", j, "/", orders[i],".control.txt", sep = ""))
    
    # run ChromEvol
    system2(command = "../mod.comp/chromEvol.exe",
            args = paste("../mod.comp/", orders[i],"/", j, "/", orders[i],".control.txt", sep = ""),
            wait = T)
  }
}
