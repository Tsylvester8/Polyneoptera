...
for(j in 1:100){
  ...
  if(!dir.exists(paths = paste("../mod.comp/", "Blattodea_sensu_stricto","/",j, sep = ""))){
    dir.create(path = paste("../mod.comp/", "Blattodea_sensu_stricto","/",j, sep = ""), 
               recursive = T)
  }
  
  write.tree(phy = tree.pruned, 
             file = paste("../mod.comp/", "Blattodea_sensu_stricto", "/", j, "/", "Blattodea_sensu_stricto",".tre", sep = ""))
  ...
  write.table(counts, 
              file = paste("../mod.comp/", "Blattodea_sensu_stricto","/", j, "/", "Blattodea_sensu_stricto",".count.txt", sep = ""),
              quote = F,
              sep = "\n",
              row.names = F,
              col.names = F)
  ...
  make.control(mainType = "Run_Fix_Param",
               outDir = paste("../mod.comp/", "Blattodea_sensu_stricto","/",j,"/","results", sep = ""),
               dataFile = paste("../mod.comp/", "Blattodea_sensu_stricto","/", j, "/", "Blattodea_sensu_stricto",".count.txt", sep = ""),
               treeFile = paste("../mod.comp/", "Blattodea_sensu_stricto", "/", j, "/", "Blattodea_sensu_stricto",".tre", sep = ""),
               maxChrNum = rng[2],
               minChrNum = rng[1],
               branchMul = 1,
               simulationsNum = nsim,
               ...
               control.path = paste("../mod.comp/", "Blattodea_sensu_stricto","/", j, "/", "Blattodea_sensu_stricto",".control.txt", sep = ""))
  
  # run ChromEvol
  system2(command = "../mod.comp/chromEvol.exe",
          args = paste("../mod.comp/", "Blattodea_sensu_stricto","/", j, "/", "Blattodea_sensu_stricto",".control.txt", sep = ""),
          wait = T)
}
