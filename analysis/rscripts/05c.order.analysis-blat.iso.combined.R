...
results <- foreach(j=1:100) %dopar% {
    ...
    tree <- keep.tip(phy = tree, tip = dat$species[dat$order %in% c("Blattodea", "Isoptera")])
    tree.depth <- max(branching.times(tree))
    # scale tree to unit length
    tree$edge.length <- tree$edge.length/max(branching.times(tree))
    # create reduced dataset
    dat.t <- dat[dat$order %in% c("Blattodea", "Isoptera"), ]
    # track progress
    ...
    results.trees <- mcmc(con.lik, w = 1,
                          x.init = runif(min = 0, max = 1, n = 3), 
                          nsteps = iter, upper = 100)
    # scale units to millions of years
    results.trees[,2:4] <- results.trees[,2:4] / tree.depth
    results.trees
  }

# save the results of running this analysis
save.image("../results/05c.blat.iso.comb.rates.RData")
